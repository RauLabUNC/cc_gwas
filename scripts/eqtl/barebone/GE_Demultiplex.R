...
#Save the initially demultiplexed raw counts
write.csv(summarized_counts, "All_Raw_Counts.csv")
...
# Import reference table with sample info, ID, plate number, barcode, etc
sampleInfoBasic <- read.csv("sampleInfoBasic.csv")
...
counts_2 = as.data.frame(t(summarized_counts)) #now make gene names as columns, samples as rows
...
#Save the clean counts, this will be used in script GE_Transform.R
write.csv(Counts_and_Info, "Demultiplexed_Raw_Count.csv")